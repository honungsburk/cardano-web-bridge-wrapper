...
test('getRewardAddress can handle address', async () => {
  const wallet = createWallet((walletEnabledAPI) => {
    walletEnabledAPI.getRewardAddress = async () =>
      toHex(
        RewardAddress.new(0, StakeCredential.from_scripthash(mkScriptHash(1)))
          .to_address()
          .to_bytes(),
      );
  });
...
export const toAssetName = (hexString: string) => {
  const assetName: Uint8Array = Uint8Array.from(Buffer.from(hexString, 'hex'));
...
export const mkScriptHash = (scriptNumber: number) => {
  const script = lib.NativeScript.new_timelock_start(lib.TimelockStart.new(scriptNumber)).hash(0);
 ...
 * @returns an address
 */
export const mkAddress = (index = 0) => {
  const addresses = [
...
export const mkValue = (ada: BigNum, nativeAssets: SimpleNativeAssets) => {
  const value = lib.Value.new(ada);
  const mulAssets = lib.MultiAsset.new();
  nativeAssets.forEach((asset) => {
    const assets = lib.Assets.new();
    asset.assets.forEach((namedAsset) => assets.insert(namedAsset.assetName, namedAsset.amount));
